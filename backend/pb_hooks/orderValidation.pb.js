...
onRecordCreate((e) => {
  try {
    ...
    // Re-throw intentional validation errors as-is (their message is meant to
    // reach the client). Anything else is an unexpected bug in this hook — log
    // it AND surface it in the response instead of letting PocketBase collapse
    // it into an opaque generic "Failed to create record." with no detail.
    if (err instanceof BadRequestError) {
      throw err;
    }
    $app.logger().error("Order create: unexpected error in pricing validation", "error", String(err));
    throw new BadRequestError("Order validation failed: " + String(err));
  }
}, "orders");
